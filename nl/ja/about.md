...
{{site.data.keyword.Db2_on_Cloud_long}} は、完全マネージド SQL データベース・サービスです。 {{site.data.keyword.Db2_on_Cloud_short}} Web コンソールを使用すると、スプレッドシートのインポート、SQL ステートメントの実行、サービスのモニターなど、一般的なタスクを簡単に行うことができます。
{: shortdesc}

## 管理対象サービスに含まれるもの
{: #managed_service}

{{site.data.keyword.Db2_on_Cloud_short}} の完全マネージド・サービスは、数回のクリックでデプロイし、簡単にデータベースを実行できます。 これは、既にユーザーのために最適化されています。 サービスは、その下にあるソフトウェアのアップグレードとハードウェアの問題を処理します。 このサービスには、データベースおよびインフラストラクチャーの 24 時間 365 日ヘルス・モニターが含まれています。 ディスク上のデータは暗号化され、強化されたセキュリティーでデータベースに接続されます。 高可用性のプランは、シームレスなローリング更新を提供し、ダウン時間なしでアプリケーションを拡張できます。 詳しくは、[高可用性](../Db2onCloud/ha.html){:new_window}を参照してください。
{: shortdesc}
