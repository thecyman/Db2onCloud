...
Pour plus d'informations sur la virtualisation des données (fédération), voir [Federation](https://www.ibm.com/support/knowledgecenter/SSFMBX/com.ibm.swg.im.dashdb.doc/fcontainer.html){:external}.

Par défaut, {{site.data.keyword.Db2_on_Cloud_short}} prend en charge Informix et Db2 (y compris Db2 et Db2 Warehouse sur site). La prise en charge de certaines sources de données devra peut-être être installée par le support IBM sur demande. Pour plus d'informations sur les sources de données prises en charge par la fédération, voir [Federation Supported Data Sources](https://www.ibm.com/support/docview.wss?uid=swg27050561){:external}.
