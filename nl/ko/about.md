...
{{site.data.keyword.Db2_on_Cloud_long}}는 완전 관리형 SQL 데이터베이스 서비스입니다. {{site.data.keyword.Db2_on_Cloud_short}} 웹 콘솔을 사용하면 공통 태스크(예: 스프레드시트 가져오기, SQL문 실행, 서비스 모니터링)가 쉬워집니다.
{: shortdesc}

## 관리 서비스에 포함된 내용
{: #managed_service}

{{site.data.keyword.Db2_on_Cloud_short}}의 완전 관리형 서비스는 몇 번의 클릭으로 배치되고, 사용자는 쉽게 데이터베이스를 실행할 수 있습니다. 이미 사용자에 맞게 최적화되어 있습니다. 서비스는 기본 소프트웨어 업그레이드 및 하드웨어 문제를 처리합니다. 서비스에는 데이터베이스 및 인프라에 대한 상태 모니터링을 하루 24시간, 일주일 내내 수행하는 작업이 포함됩니다. 디스크에 기록된 데이터는 암호화되고 데이터베이스 연결에는 강화된 보안이 설정됩니다. 고가용성 플랜은 원활한 롤링 업데이트를 제공하고 작동 중단 없이 앱을 스케일링할 수 있습니다. 자세한 정보는 [고가용성](../Db2onCloud/ha.html){:new_window}을 참조하십시오.
{: shortdesc}
