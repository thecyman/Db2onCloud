...
[무료 Db2 Developer Edition 다운로드](https://www.ibm.com/us-en/marketplace/ibm-db2-direct-and-developer-editions){:external}를 사용하여 로컬 Db2 데이터베이스를 설치할 수도 있습니다. 이는 Docker 컨테이너 내에 도구를 포함하는 즉시 사용 가능한 Db2 개발자 에디션을 빠르게 설치합니다(Docker는 필요하지 않으며 필요한 컴포넌트는 자동으로 설치됨). 
...
MacOS용 Python 드라이버 패키지를 설치하려면 `pip` 명령을 `--no-cache-dir` 옵션과 함께 사용해야 합니다. 자세한 지시사항은 [Python support for IBM DB2 and IBM Informix](https://ibm.biz/db2-drivers-python){:external}를 참조하십시오.
{: note}

Python을 사용하여 데이터 과학 작업을 수행 중이거나 데이터 프레임, In-Database 분석 또는 Watson Studio 사용에 대한 지원을 원하는 경우 다양한 데이터 과학 기능에 대한 지원에 초점을 맞춘 대체 드라이버 [ibmdbpy 0.1.5](https://pypi.org/project/ibmdbpy/){:external}를 사용하려고 할 수 있습니다.
{: note}
...
- [Python 드라이버](https://ibm.biz/db2-drivers-python){:external}
- [Node.js 드라이버](https://ibm.biz/db2-drivers-node){:external}
...
1. 앱 또는 도구가 실행 중인 컴퓨터에서 [드라이버 패키지를 설치](https://www.ibm.com/support/knowledgecenter/SSFMBX/com.ibm.swg.im.dashdb.doc/connecting/connect_driver_package_install.html){:external}하십시오.
2. {{site.data.keyword.Db2_on_Cloud_short}} 데이터베이스에 대한 [드라이버 파일을 구성](https://www.ibm.com/support/knowledgecenter/en/SSFMBX/com.ibm.swg.im.dashdb.doc/connecting/connect_driver_package_config.html){:external}하십시오.
