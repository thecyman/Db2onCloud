...
# Informationen zu Db2 on Cloud
{: #overview}

{{site.data.keyword.Db2_on_Cloud_long}} ist ein vollständig verwalteter SQL-Datenbankservice. Mit der {{site.data.keyword.Db2_on_Cloud_short}}-Webkonsole können gängige Aufgaben, wie z. B. das Importieren von Arbeitsblättern, das Ausführen von SQL-Anweisungen und das Überwachen des Service, auf unkomplizierte Weise durchgeführt werden.
{: shortdesc}

## Welche Funktionen umfasst der verwaltete Service?
{: #managed_service}

Der vollständig verwaltete {{site.data.keyword.Db2_on_Cloud_short}}-Service kann mit wenigen Mausklicks bereitgestellt werden und Sie können die Datenbank problemlos ausführen. Die Optimierung ist bereits erfolgt. Der Service erledigt alle zugrunde liegenden Software-Upgrades und Hardwareprobleme automatisch. Der Service umfasst die 24x7-Statusüberwachung der Datenbank und der Infrastruktur. Ihre auf Platte gespeicherten Daten werden verschlüsselt und die Verbindungen zur Datenbank werden mit erweiterten Sicherheitsfunktionen hergestellt. Hochverfügbarkeitspläne bieten nahtlose rollierende Aktualisierungen und Sie können Ihre Apps ohne Ausfallzeiten skalieren. Weitere Informationen finden Sie im Abschnitt [Hochverfügbarkeit](../Db2onCloud/ha.html){:new_window}.
{: shortdesc}
