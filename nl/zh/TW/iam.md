...
如果是要使用 IAM 鑑別連接至 Db2 伺服器的 ODBC 應用程式或指令行用戶端 (CLPPLUS)，必須執行下列指令，先在 `db2dsdriver.cfg` 設定檔中設定資料來源名稱 (DSN)：
...
* 若要新增 IBM ID，請執行下列範例 API 呼叫：
  ...
  **附註**：`"id"` 及 `"ibmid"` 的 `<userid>` 值不必相同。這兩個不同的 ID 不會以任何方式鏈結在一起。
