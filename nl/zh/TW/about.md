...
{{site.data.keyword.Db2_on_Cloud_long}} 是完全受管理的 SQL 資料庫服務。{{site.data.keyword.Db2_on_Cloud_short}} Web 主控台讓一般作業變輕鬆，例如匯入試算表、執行 SQL 陳述式，以及監視服務。
{: shortdesc}

## 受管理服務中包含什麼？
{: #managed_service}

只要按幾下滑鼠即可部署 {{site.data.keyword.Db2_on_Cloud_short}} 的完全受管理服務，且您可以執行資料庫而無困擾。它已經為您進行最佳化了。服務會為您處理任何基礎的軟體升級及硬體問題。服務包含對於資料庫及基礎架構的全天候性能監視。您的磁碟內存資料會經過加密，與資料庫的連線也會具有加強安全。高可用性方案提供平順的漸進式更新，您可以調整應用程式而不必關閉。如需相關資訊，請參閱[高可用性](../Db2onCloud/ha.html){:new_window}。
{: shortdesc}
