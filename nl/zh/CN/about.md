...
{{site.data.keyword.Db2_on_Cloud_long}} 是完全受管的 SQL 数据库服务。{{site.data.keyword.Db2_on_Cloud_short}} Web 控制台简化常见任务，例如，导入电子表格、运行 SQL 语句和监视服务。
{: shortdesc}

## 受管服务中包含什么？
{: #managed_service}

{{site.data.keyword.Db2_on_Cloud_short}} 完全受管服务可在几次单击内完成部署，而且您可以无障碍地运行自己的数据库。已针对您进行优化。此服务为您处理任何底层软件升级和硬件问题。该服务全天不间断监视数据库和基础架构的运行状况。磁盘上的数据进行加密，并在增强安全的情况下建立数据库连接。高可用性套餐提供无缝滚动更新，您可以在不停机的情况下扩展应用程序。有关更多信息，请参阅[高可用性](../Db2onCloud/ha.html){:new_window}。
{: shortdesc}
